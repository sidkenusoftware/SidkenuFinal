...
namespace Sidkenu.Servicio.Implementacion.Core
{
    ...
    {
        private readonly IValidator<MotivoBajaPersistenciaDTO> _validatorDto;

        public MotivoBajaServicio(IUnidadDeTrabajo unitOfWork,
                                 IMapper mapper,
                                 ILogger logger,
                                 IConfiguracionServicio configuracionServicio,
                                 IValidator<MotivoBajaPersistenciaDTO> validatorDto)
        ...
        public ResultDTO Add(MotivoBajaPersistenciaDTO entidad, string user)
        {
            try
            {
                ...
                if (!validationResult.IsValid)
                {
                    ...
                    {
                        _logger.Information($"Error de validacion de Datos de la MotivoBaja: {mensaje}");
                    }
                ...
                var entity = _mapper.Map<Dominio.Entidades.Core.MotivoBaja>(entidad);

                entity.User = user;
                entity.EstaEliminado = false;

                _unitOfWork.MotivoBajaRepository.Add(entity);
        ...
        public ResultDTO Update(MotivoBajaPersistenciaDTO entidad, string user)
        {
            try
            {
                ...
                var entityActual = _unitOfWork.MotivoBajaRepository.GetById(entidad.Id);

                if (entityActual == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information("Ocurrio un error al obtener los datos de la MotivoBaja", entidad);
                    }
                ...
                _unitOfWork.MotivoBajaRepository.Update(entity);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Update MotivoBaja - User: {user}", entity);
                }
        ...
        public ResultDTO Delete(MotivoBajaDeleteDTO deleteDTO, string user)
        {
            try
            {
                var entidad = _unitOfWork.MotivoBajaRepository.GetById(deleteDTO.Id);

                if (entidad == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la MotivoBaja. Id: {deleteDTO.Id}", entidad);
                    }
                ...
                _unitOfWork.MotivoBajaRepository.Delete(deleteDTO.Id, user);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Delete MotivoBaja - User: {user}", entidad);
                }
        ...
        public ResultDTO GetAll()
        {
            try
            {
                var entities = _unitOfWork.MotivoBajaRepository.GetAll();
        ...
        public ResultDTO GetAll(Guid empresaId)
        {
            try
            {
                Expression<Func<Dominio.Entidades.Core.MotivoBaja, bool>> filtro = filtro => true;

                filtro = filtro.And(x => x.EmpresaId == empresaId || x.EmpresaId == null);

                var entities = _unitOfWork.MotivoBajaRepository.GetByFilter(filtro);
        ...
        public ResultDTO GetById(Guid id)
        {
            try
            {
                var entity = _unitOfWork.MotivoBajaRepository.GetById(id);

                if (entity == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la MotivoBaja. Id: {id}");
                    }
        ...
        {
            var entidads = _unitOfWork.MotivoBajaRepository.GetAll();
