...
{
    public class BancoServicio : ServicioBase, IBancoServicio
    {
        private readonly IValidator<BancoPersistenciaDTO> _validatorDto;

        public BancoServicio(IUnidadDeTrabajo unitOfWork,
                               IMapper mapper,
                               ILogger logger,
                               IConfiguracionServicio configuracionServicio,
                               IValidator<BancoPersistenciaDTO> validatorDto)
                               : base(unitOfWork, mapper, logger, configuracionServicio)
        {
            _validatorDto = validatorDto;
        }

        public ResultDTO Add(BancoPersistenciaDTO entidad, string user)
        {
            try
            {
                ...
                if (!validationResult.IsValid)
                {
                    ...
                    {
                        _logger.Information($"Error de validacion de Datos de la Banco: {mensaje}");
                    }
                ...
                var entity = _mapper.Map<Dominio.Entidades.Core.Banco>(entidad);

                entity.User = user;
                entity.EstaEliminado = false;

                _unitOfWork.BancoRepository.Add(entity);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Add Banco - User: {user}", entity);
                }
        ...
        public ResultDTO Update(BancoPersistenciaDTO entidad, string user)
        {
            try
            {
                var validationResult = _validatorDto.Validate(entidad);
                ...
                var entityActual = _unitOfWork.BancoRepository.GetById(entidad.Id);

                if (entityActual == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information("Ocurrio un error al obtener los datos de la Banco", entidad);
                    }

                    return new ResultDTO
                    {
                        Message = "Ocurrio un error al obtener los datos de la Banco",
                        State = false
                    };
                ...
                _unitOfWork.BancoRepository.Update(entity);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Update Banco - User: {user}", entity);
                }

                return new ResultDTO
                {
                    State = true,
                    Data = _mapper.Map<BancoDTO>(entity),
        ...
        public ResultDTO Delete(BancoDeleteDTO deleteDTO, string user)
        {
            try
            {
                var entidad = _unitOfWork.BancoRepository.GetById(deleteDTO.Id);

                if (entidad == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la Banco. Id: {deleteDTO.Id}", entidad);
                    }
                ...
                _unitOfWork.BancoRepository.Delete(deleteDTO.Id, user);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Delete Banco - User: {user}", entidad);
                }
        ...
        public ResultDTO GetAll()
        {
            try
            {
                var entities = _unitOfWork.BancoRepository.GetAll();

                return new ResultDTO
                {
                    State = true,
                    Data = _mapper.Map<IEnumerable<BancoDTO>>(entities)
                };
            }
            catch (Exception ex)
            {
        ...
        public ResultDTO GetAll(Guid empresaId)
        {
            try
            {
                Expression<Func<Banco, bool>> filtro = filtro => true;

                filtro = filtro.And(x => x.EmpresaId == empresaId || x.EmpresaId == null);
                var entities = _unitOfWork.BancoRepository.GetByFilter(filtro);
        ...
        public ResultDTO GetByFilter(BancoFilterDTO filter)
        {
            try
            {
                filter.CadenaBuscar = !string.IsNullOrEmpty(filter.CadenaBuscar) ? filter.CadenaBuscar : string.Empty;

                Expression<Func<Dominio.Entidades.Core.Banco, bool>> filtro = filtro => true;
        ...
        public ResultDTO GetById(Guid id)
        {
            try
            {
                var entity = _unitOfWork.BancoRepository.GetById(id);

                if (entity == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la Banco. Id: {id}");
                    }
        ...
        {
            var entidads = _unitOfWork.BancoRepository.GetAll();
