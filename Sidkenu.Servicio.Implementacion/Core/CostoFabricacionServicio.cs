...
namespace Sidkenu.Servicio.Implementacion.Core
{
    ...
    {
        public CostoFabricacionServicio(IUnidadDeTrabajo unitOfWork,
                                        IMapper mapper,
                                        ILogger logger,
        ...
        public ResultDTO Add(CostoFabricacionPersistenciaDTO entidad, string user)
        {
            try
            {
                ...
                var entity = _mapper.Map<Dominio.Entidades.Core.CostoFabricacion>(entidad);

                entity.User = user;
                entity.EstaEliminado = false;

                _unitOfWork.CostoFabricacionRepository.Add(entity);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Add CostoFabricacion - User: {user}", entity);
                }
        ...
        public ResultDTO Update(CostoFabricacionPersistenciaDTO entidad, bool actualizarPrecio, string user)
        {
            try
            {
                var entityActual = _unitOfWork.CostoFabricacionRepository.GetById(entidad.Id);

                if (entityActual == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information("Ocurrio un error al obtener los datos de la CostoFabricacion", entidad);
                    }
                ...
                _unitOfWork.CostoFabricacionRepository.Update(entity);
                ...
                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Update CostoFabricacion - User: {user}", entity);
                }
        ...
        public ResultDTO Delete(CostoFabricacionDeleteDTO deleteDTO, string user)
        {
            try
            {
                var entidad = _unitOfWork.CostoFabricacionRepository.GetById(deleteDTO.Id);

                if (entidad == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la CostoFabricacion. Id: {deleteDTO.Id}", entidad);
                    }
                ...
                _unitOfWork.CostoFabricacionRepository.Delete(deleteDTO.Id, user);

                _unitOfWork.Commit();

                if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                {
                    _logger.Information($"Delete CostoFabricacion - User: {user}", entidad);
                }
        ...
        public ResultDTO GetAll()
        {
            try
            {
                var entities = _unitOfWork.CostoFabricacionRepository.GetAll();
        ...
        public ResultDTO GetAll(Guid empresaId)
        {
            try
            {
                Expression<Func<Dominio.Entidades.Core.CostoFabricacion, bool>> filtro = filtro => true;

                filtro = filtro.And(x => x.EmpresaId == empresaId || x.EmpresaId == null);

                var entities = _unitOfWork.CostoFabricacionRepository.GetByFilter(filtro);
        ...
        public ResultDTO GetById(Guid id)
        {
            try
            {
                var entity = _unitOfWork.CostoFabricacionRepository.GetById(id);

                if (entity == null)
                {
                    if (_configuracionDTO != null && _configuracionDTO.LogInformacion)
                    {
                        _logger.Information($"Ocurrio un error al obtener los datos de la CostoFabricacion. Id: {id}");
                    }
        ...
        {
            var entidads = _unitOfWork.CostoFabricacionRepository.GetAll();
